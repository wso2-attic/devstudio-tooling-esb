...
//			}
		...
		if (!mediatorRestricted) {
			...
			if (previousMediator == null && nearestOutputConnector == null && nearestESBLink == null
					&& nearestInputConnector == null) {
				// check whether previous mediator is a Drop Mediator
				// TODO:- Validate adding drop after drop.
				if (checkForDropMediator(inputConnectorEditpartList) && !(this instanceof DropMediatorEditPart)) {
					mediatorRestricted = true;
					mediatorType = "Drop Mediator";
...
/*				if((EditorUtils.getMediator(((EsbLinkEditPart)element).getSource())!=null)&&
	 ...
	 * @return - True if it is a drop mediator.
	 */
	private boolean checkForDropMediator(ArrayList<AbstractConnectorEditPart> connectors) {
