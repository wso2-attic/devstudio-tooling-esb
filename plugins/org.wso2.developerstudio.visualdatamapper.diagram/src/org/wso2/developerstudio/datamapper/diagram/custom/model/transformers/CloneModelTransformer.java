...
public class CloneModelTransformer implements IModelTransformer {

	@Override
	public DMOperation transform(OperatorImpl operator, int index) {
		if (operator instanceof CloneImpl) {
			DMOperation addOperatorModel = new DMOperation(DataMapperOperatorType.CLONE, operator.toString(), index);
			return addOperatorModel;
		}
		throw new IllegalArgumentException("CloneImpl operator expected. Found : " + operator.toString());
	}
}
